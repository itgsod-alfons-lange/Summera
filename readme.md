...
Den här uppgiften går ut på att skriva ett program som returnerar en summa av en talserie.
...
Du skall skriva en funktion: `summera` som tar två parametrar (`start` och `stop`)

Funktionen skall sen returnera ut summan av alla tal från och med starttalet till och med sluttalet.
...
#### Ruby ####
	...
	summera(start: 3, stop: 5)
	#=> 12

	summera(start: 2, stop: -5)
	#=> -12
...
Skapa funktionerna i `lib/summera.rb`

Testerna finns i `spec/summera_spec.rb`

Kör `rspec` för att köra testerna.

##### Python #####

Skapa funktionerna i `lib/summera.py`

Testerna finns i `test/summera_test.py`
